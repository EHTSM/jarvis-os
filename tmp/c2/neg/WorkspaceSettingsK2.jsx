...
function TokensPanel() {
  // C.2 (C2-03): A.11.8 recovered destructive confirmation for the Sessions and
  // (other) panels in this same file, but the Tokens panel was missed. Revoking
  // an API token is irreversible and immediately breaks any integration using
  // it — a strictly more damaging action than revoking a session, which DID
  // confirm. Same file, same established useConfirm/ConfirmDialog pattern.
  const [confirm, ConfirmUI] = useConfirm();
  ...
  async function revoke(id) {
    ...
    if (!await confirm({
      ...
      message: `${t?.name || "That token"} will stop working immediately. Any script or integration using it will start failing. This cannot be undone.`,
      danger: true,
      confirmLabel: "Revoke",
    })) return;
    try {
