...
export default function AIBenchmarkLab() {
  ...
  const runBench = async () => {
    ...
    try {
      ...
      // C.2 (C2-02): this used to setRunResult(await r.json()) with no status
      // check. On a 401/403/500 the error body still parses, so runResult
      // became truthy and the UI rendered "✓ undefined benchmark runs
      // completed" — a green success tick for an action that FAILED. A user
      // would believe the benchmark ran. Failures now go to the error state
      // that is already rendered directly above the success banner.
      if (!r.ok) {
        let detail = "";
        try { const b = await r.json(); detail = b?.error || b?.message || ""; } catch { /* non-JSON body */ }
