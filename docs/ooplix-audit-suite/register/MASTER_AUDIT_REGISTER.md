...
| **B.19.5** | **9.0** | **WCAG 2.2 AA final certification.** 0 axe AA violations across 10 authenticated runs (5 viewports × 2 themes); live composited contrast 0 both themes; 6/6 critical journeys pass. **CONDITIONAL** — `G1-B193` (763 form-label findings) is a live-confirmed 3.3.2 failure and screen-reader testing was ENVIRONMENT BLOCKED. |
| **B.19.6** | **7.5** | **Screen-reader certification NOT AWARDED.** 0 of 20 journeys executed under any screen reader: VoiceOver BLOCKED (unsafe on a live desktop), TalkBack BLOCKED (bound and running, but the XR AVD cannot expose Chrome node tree), NVDA/Narrator/iOS NOT AVAILABLE. Accessibility tree measured instead: 0 unnamed interactive nodes across 7 surfaces x 2 themes; 1 new ErrorBoundary contrast defect found and fixed. |
...
| 1.2.0 | 2026-08-13 | Added B.19.4 (Keyboard & ARIA Recovery). Register now carries **36 executed audits**; 132 findings; mean 8.63/10 across 11 scored audits. All 77 keyboard findings closed and dialog semantics recovered (7→15). The 766 remaining form-labelling findings are reclassified from "pre-existing" to a **GENUINE CAPABILITY GAP** with per-finding evidence (`G1-B193`), and a new regression suite (`tests/runtime/28`) guards the recovered capability. | Audit Programme |

| 1.3.0 | 2026-08-13 | Added **B.19.5 (WCAG 2.2 AA Final Certification)**, recorded under 19.5 because B.19.4 is already an executed, certified row (Keyboard & ARIA Recovery) and reusing the number would overwrite it. Register now carries **36 executed audits**; mean 8.66/10 across 12 scored audits. Automated AA reaches **0 violations** across 10 authenticated axe runs (390/430/768/1024/1440 × dark+light) and **0** on the live composited contrast sweep. Seven defects fixed, two of them functional as well as accessibility defects (a CSS Grid area collision that made the Command Dispatch input largely unclickable; a skip link that updated the hash but never moved focus). Two were found only by walking critical user journeys, which reach surfaces the tab sweep never visits. `G1-B193` is **re-measured at 763** and re-confirmed live (0 visible labels, 6 placeholder-only) — it remains a GENUINE CAPABILITY GAP. New gap `G2-B195` (palette pin button is an invalid listbox child) recorded with all four candidate remedies tested against axe. Certification is **CONDITIONAL, not 10/10**: screen-reader testing was ENVIRONMENT BLOCKED and is not claimed in any form. | Audit Programme |
