...
function _categorizeCrash(message = "") {
  ...
  if (m.includes("eaddrinuse") || m.includes("address in use"))
    return { group: "Port conflict", explanation: "Another process is already using Ooplix's port. Restart with: pm2 restart jarvis-backend", severity: "high" };
  ...
  if (m.includes("heap out of memory") || m.includes("javascript heap"))
    return { group: "Memory exhaustion", explanation: "Ooplix ran out of memory. Restart the app or increase Node.js heap: NODE_OPTIONS=--max-old-space-size=4096", severity: "high" };
  ...
  if (m.includes("permission denied") || m.includes("eacces"))
    return { group: "Permission error", explanation: "Ooplix doesn't have permission to access a file or port. Check file ownership.", severity: "medium" };
...
const RECOVERY_STEPS = [
  ...
  },
  {
    ...
    hint: "After restarting, wait 5-10 seconds then refresh the Ooplix app. The yellow reconnecting banner should clear.",
