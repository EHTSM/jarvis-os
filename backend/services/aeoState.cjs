...
// Additive only — unset resolves byte-identical to before (real data/aeo/). When set,
// redirects to an isolated per-process subdirectory so tests/runtime/*.test.cjs suites reaching
// this layer stop writing real records into production data/aeo/ (Mission 97/98 defect class).
const DATA_DIR = process.env.JARVIS_TEST_DATA_SUFFIX
  ? path.join(__dirname, "../../data", `aeo.${process.env.JARVIS_TEST_DATA_SUFFIX}`)
  : path.join(__dirname, "../../data/aeo");
const FILES = {
