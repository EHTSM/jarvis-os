...
// _state's top-level arrays (objectives/campaigns/deals/tasks/handoffs/
// blockers/approvals/reports) previously had no cap anywhere — every one of
// the 20 business-dept ticks that runs a workflow step (CEO objective →
// marketing campaign → growth lead → CRM deal → ...) pushes to these without
// bound, growing this file and its in-memory _state object forever. Cap each
// on save, same pattern as executiveState.cjs/akoState.cjs.
const STATE_ARRAY_CAPS = {
  objectives: 500, campaigns: 1000, deals: 3000, tasks: 3000,
  handoffs: 1000, blockers: 1000, approvals: 1000, reports: 500,
};
...
function _save() {
  ...
  if (_reports.length > 500)  _reports.splice(0, _reports.length - 500);
...
function createDeal({ title, company, contactEmail, value = 0, stage = "prospect", deptId = "bizorg_crm", campaignId, leadSource, synthetic = false } = {}) {
  if (!title) return { ok: false, error: "title required" };
  _s();
  const deal = {
    id:           _id("bdeal"),
    ...
    createdAt:    new Date().toISOString(),
    updatedAt:    new Date().toISOString(),
    // Zero-Trust Competitor Remediation, Phase 2: explicit honesty flag —
    // true only for deals generated by the autonomous demo tick
    // (businessOrg.cjs's _growthTick), never for real HTTP-triggered lead
    // capture. See businessOrgWorkflow.cjs's growthCaptureLead() for the
...
function getDashboard() {
  ...
  // cascade — see growthCaptureLead()'s comment for the full incident) —
  // a real consumer of this data (dashboard UI, export, partner
  // integration) had no way to distinguish simulated activity from real
  // customer activity. dataIntegrity makes that split explicit rather
  // than leaving it ambiguous.
  const syntheticDeals = deals.filter(d => d.synthetic);
  const realDeals      = deals.filter(d => !d.synthetic);
  return {
    updatedAt:   new Date().toISOString(),
    quarter:     _q(),
    ...
    dataIntegrity: {
      ...
      syntheticDeals: syntheticDeals.length,
      realDeals:      realDeals.length,
      note: syntheticDeals.length > 0
        ? `${syntheticDeals.length} of ${deals.length} deals in this dashboard were generated by the autonomous demo simulation (businessOrg.cjs's growth/CRM tick), not real customer activity.`
        : "No synthetic/demo deals present.",
    },
