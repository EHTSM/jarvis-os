...
function _writeRepoScaffold(blueprint, repos, wsId) {
  const name    = blueprint.name.toLowerCase().replace(/\s+/g, "-");
  ...
  for (const repo of repos.repositories) {
    ...
    const readme = `# ${repo.name}\n\n${repo.description}\n\nStack: ${repo.stack || "n/a"}\n\nGenerated by companyWorkspaceBuilder for blueprint "${blueprint.name}" (${blueprint.templateId}).\nThis is a real starter scaffold, not a complete application — the files\nlisted in the workspace's \`repositories.files\` map beyond README/package.json\nare declared structure, not yet generated content (no AI code-scaffolding\nengine exists in this codebase to fill them in).\n`;
  ...
  return { ok: true, wsDir: path.relative(ROOT, wsDir), filesWritten: written };
}
...
async function buildWorkspace(blueprintId) {
  ...
  const scaffold = _writeRepoScaffold(blueprint, repos, wsId);
  repos.scaffoldPath  = scaffold.wsDir;
  repos.filesWritten  = scaffold.filesWritten;
  _step("repositories", { count: repos.repositories.length, filesWritten: scaffold.filesWritten.length, scaffoldPath: scaffold.wsDir });
